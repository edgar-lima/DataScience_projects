...
4. _Normalidade dos resíduos_: o pressuposto de normalidade dos resíduos também é checado de maneira gráfica, onde no eixo x colocado os quantis e no eixo y é colocado os resíduos padronizados.
```{r, message=F}
car::qqPlot(model)
```
Os resíduos são considerados normais quando é identificado um padrão de realçao linear positiva entre o eixo x e y. Com isso podemos concluir que os resíduos do modelo em questão não segue uma distribuição normal.
